      ...
      items: [
        {
          ...
          pageNo: '',
          additionalInfo: `Declarations of the International Labor Organisation (ILO), including the employee right to the freedom of association and the right to engage in collective bargaining, LBF's strong relationships with employees has meant there is no need for any collective agreement. During the year under review, the Company has not faced any type of industrial disputes.`,
          link: null,
        },
        {
                  ...
                  {category.items.map((item, itemIndex) => (
                    <React.Fragment key={`${index}-${itemIndex}`}>
                      <tr className="bg-blue-700">
                        ...
                        <td className="border border-white p-2">
                          {item.link ? (
                            <a href={item.link}>{item.pageNo}</a>
                          ) : (
