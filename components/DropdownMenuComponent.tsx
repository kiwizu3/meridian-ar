    ...
    <div>
      {item?.children ? (
          ...
          <DropdownTrigger aria-label="integrated-dashboard">
            <button
              ...
              className={`cursor-pointer focus-visible:outline-none flex gap-2 items-center font-secondary font-medium leading-[19.54px] text-base hover:text-red ${
                isDropdownActive() ? 'text-red' : 'text-white'
              }`}
            >
              {item?.title}
            ...
            {item?.children?.map((child: any) => (
              <DropdownItem textValue={child.key} key={child.key}>
                <SpeechText>
                  <Link prefetch={false}
                    href={child.url}
                    className={`font-secondary font-medium leading-[19.54px] text-base hover:text-red transition ease-in-out duration-300 ${isActive(child?.url) ? 'text-red' : 'text-white'}`}
                  >
                    {child.title}
                  </Link>
                </SpeechText>
        ...
        <SpeechText>
          <Link prefetch={false}
            href={item?.url}
            className={`font-secondary font-medium leading-[19.54px] text-base hover:text-red transition ease-in-out duration-300 ${isActive(item?.url) ? 'text-red' : 'text-white'}`}
          >
            {item?.title}
          </Link>
        </SpeechText>
